...
def display_tasks():
    ...
    for row in c.execute('SELECT * FROM tasks WHERE status = ?', ('未完了',)):
        task_listbox.insert(tk.END, row)

# 過去のタスクを表示する関数
def show_past_tasks():
    past_tasks_window = tk.Toplevel(root)
    past_tasks_window.title("過去のタスク")

    past_task_listbox = tk.Listbox(past_tasks_window, width=50, height=15)
    past_task_listbox.pack(pady=10)

    for row in c.execute('SELECT * FROM tasks WHERE status = ?', ('完了',)):
        past_task_listbox.insert(tk.END, row)
...
show_past_tasks_button = tk.Button(root, text="過去のタスクを表示", command=show_past_tasks)
show_past_tasks_button.pack(pady=5)
